...
; CHANGELOG:
;
; Sep 18 2024: Updated for AutoHotkey v2.
...
Result := InputBox("Provide the corrected word on the right side. You can also edit the left side if you wish.`n`nExample entry:`n::teh::the", "New Hotstring", ,"::%Hotstring%::%Hotstring%")


if Result.Result = "Cancel" or Result.Result = "Timeout" ; The user pressed Cancel.
  return
Hotstring := Result.Value
; Otherwise, add the hotstring and reload the script:
FileAppend("`n%Hotstring%", "%A_ScriptFullPath%")  ; Put a `n at the beginning in case file lacks a blank line at its end.
Reload
Sleep 200 ; If successful, the reload will close this instance during the Sleep, so the line below will never be reached.
Result := MsgBox("The hotstring just added appears to be improperly formatted.  Would you like to open the script for editing? Note that the bad hotstring is at the bottom of the script.", "", 4)
if Result = "yes"
  Edit
return
}
